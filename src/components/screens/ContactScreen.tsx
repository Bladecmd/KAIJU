...
export const ContactScreen: React.FC<ContactScreenProps> = ({ onAddLog }) => {
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      {/* Header */}
      <div className="border-b border-[#1c2736] pb-6 space-y-3">
        <div className="inline-flex items-center gap-2 rounded-full border border-[#98cbff]/30 bg-[#98cbff]/10 px-3 py-1 text-xs font-mono text-[#98cbff]">
          ...
          START A CONVERSATION // DIRECT INBOUND
        </div>
        <h1 className="text-2xl md:text-3xl font-bold text-white font-mono">
          Have an interesting problem? Let’s see if I can help.
        </h1>
        <p className="text-sm text-[#a3b1c2] max-w-3xl leading-relaxed">
          Whether you have an automation system that's stalling, need hands-on technical leadership for a fast-moving build, or want to explore full-time engineering and solutions roles — drop me a line. No agency bloat, no corporate runaround.
        </p>
        ...
        <div className="pt-2 flex flex-wrap gap-2">
          {[
            ...
            'Fractional CTO / Technical Ops',
            'Commercial Advisory & Due Diligence',
            'Custom System Builds'
          ].map((tag, i) => (
            <span
              key={i}
              className="text-[11px] font-mono px-2.5 py-1 rounded-md bg-[#162231] border border-[#233548] text-[#98cbff]"
            >
              ✓ {tag}
            </span>
          ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        {/* Right: Direct Conversation & Inbound Portal */}
        <div className="lg:col-span-7">
          <div className="rounded-2xl border border-[#223142] bg-[#0c131d]/90 p-6 md:p-8 space-y-6 shadow-xl font-mono text-xs">
            <div className="border-b border-[#1c2736] pb-4">
              ...
              <p className="text-[#8ca3b8] mt-1 font-sans">
                Drop a note regarding a role, technical advisory, system build, or collaboration inquiry.
              </p>
            </div>
            ...
            {formSent ? (
              ...
              <form onSubmit={handleSubmitMessage} className="space-y-4">
                ...
                <button
                  type="submit"
                  className="w-full flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-[#98cbff] to-[#76aae6] py-3.5 text-sm font-bold text-[#001f3f] hover:opacity-95 transition-all shadow-lg shadow-[#98cbff]/20 cursor-pointer"
                >
